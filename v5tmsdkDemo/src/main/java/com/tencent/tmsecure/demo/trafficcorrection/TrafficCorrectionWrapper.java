...
import tmsdk.bg.module.network.DetailItemInfo;
...
public class TrafficCorrectionWrapper {
	...
	public void init(Context context) {
		...
		mTcMgr.setTrafficCorrectionListener(new ITrafficCorrectionListener() {
			...
			public void onCorrectionResult(int simIndex, int retCode) {
				if (mTrafficCorrectionListener != null) {
					mTrafficCorrectionListener.onCorrectionResult(simIndex, retCode);
				}
			}

			@Override
			public void onDetailInfoNotify(ArrayList<DetailCategoryInfo> detailList){
				Log.d("jiejie-detail","onDetailInfoNotify");
				if(mTrafficCorrectionListener != null && detailList != null){
					for(int m = 0; m < detailList.size(); m ++){
						DetailItemInfo fatherInfo = detailList.get(m).mFatherInfo;
						Log.d("jiejie-detail", "father[" + m + "] des is " + fatherInfo.mDescription +
								", left is " + fatherInfo.mLeft + ", used is " + fatherInfo.mUsed);

						for(int n = 0; n < detailList.get(m).mSonInfoList.size(); n ++){
							DetailItemInfo sonInfo = detailList.get(m).mSonInfoList.get(n);
							Log.d("jiejie-detail", "son[" + n + "] des is " + sonInfo.mDescription +
									", left is " + sonInfo.mLeft + ", used is " + sonInfo.mUsed);
						}
					}
				}
			}
	...
	public ArrayList<CodeName> getAllBrands(String carryId) {
		return mTcMgr.getAllBrands();
	}
